...
namespace ChatSystem
{
    public class WebSocketHandler
    {
        ...
        public WebSocketHandler(MessageStore messageStore)
        {
        ...
        public async Task ReceiveMessages(WebSocket socket, string userId)
        {
            ...
            while (socket.State == WebSocketState.Open)
            {
                ...
                if (result.MessageType == WebSocketMessageType.Text)
                {
                    ...
                    try
                    {
                        message = System.Text.Json.JsonSerializer.Deserialize<Message>(messageString);
                    }
                    catch (Exception)
                    {
                        await SendMessageToRecipient(socket, new Message
                        {
                            FromUserId = "system",
                            ToUserId = userId,
                    ...
                    if (message.MessageContent == "typing...") 
                    {
                        ...
                        if (_connections.TryGetValue(message.ToUserId, out var recipientSockets))
                        {
                            ...
                            {
                                await SendMessageToRecipient(recipientSocket, new Message
                                {
        ...
        public async Task SendMessageToRecipient(WebSocket socket, Message message)
        {
